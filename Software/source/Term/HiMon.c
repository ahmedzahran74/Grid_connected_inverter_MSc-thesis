...
inline uint16_t UartRec_Add_Char(uint8_t data) {
	uart_cmd[uart_pos / 4] = (uart_cmd[uart_pos / 4]
			& (~(((uint32_t) (0x00FF)) << 8 * (3 - (uart_pos % 4)))))
			| (((uint32_t) (data)) << 8 * (3 - (uart_pos % 4)));
...
inline uint16_t UartGetNthU16(uint32_t *uart_cmd, uint16_t nth) {
	// MSB first
	return ((uint16_t) (UartGetNthU8(uart_cmd, nth)) << 8)
			+ UartGetNthU8(uart_cmd, nth + 1);
}
...
inline uint32_t UartGetNthU32(uint32_t *uart_cmd, uint16_t nth) {
	// MSB first
	return (uint32_t) (UartGetNthU16(uart_cmd, nth + 2))
			+ (((uint32_t) UartGetNthU16(uart_cmd, nth)) << 16);
}
...
uint8_t Serial_cmd_mng(void) {
	...
	if (MON_RD == mode) /*! Read command arrived */
	{
		...
		return SER_RDTABS;
	} else if ((MON_TABVALUES(0) <= mode)
			&& (MON_TABVALUES(MON_MAXTABS) > mode)) {
		uart_cmd[0] |= 0x80000000;
		uart_tcrc = 0xFFFF;
		uart_tmt_data(STX);
		uart_tmt_datadle(&uart_tcrc, UartGetNthU8(uart_cmd, 0));
		...
		uart_tmt_len = (UartGetNthU8(uart_cmd, 9) << 8)
				+ UartGetNthU8(uart_cmd, 10);
		len = uart_tmt_len * 2 + 4 + 2;
		uart_tmt_datadle(&uart_tcrc, len >> 8);
		uart_tmt_datadle(&uart_tcrc, len & 0xFF);
		uart_bbx_1st_data = (((uint32_t) UartGetNthU16(uart_cmd, 5)) << 16)
				+ (uint32_t) UartGetNthU16(uart_cmd, 7);
...
void Serial_mng(void) {
			...
			if (uart_dlewas) {
				...
				if (uart_data == DLE) {
					...
					if (uart_data == ETX) {
						if (uart_crc != 0)
							crc_error++;
						if (UartGetNthU16(uart_cmd, 3) + 7 != uart_pos)
							length_error++;
						if ((uart_crc == 0)
								&& (UartGetNthU16(uart_cmd, 3) + 7 == uart_pos)) // good CRC, length
								{
	...
	case SER_RDTABS:
		if (UART_TMT_NOEMPTY() > 10) {
			if (uart_tmt_pos < uart_tmt_len) {
				uart_tmt_datadle(&uart_tcrc,
						((uint16_t*) (pTab))[uart_tmt_pos] >> 8);
				uart_tmt_datadle(&uart_tcrc,
						((uint16_t*) (pTab))[uart_tmt_pos] & 0xFF);
				uart_tmt_pos++;
			} else {
				tmp16 = uart_tcrc;
				uart_tmt_datadle(&uart_tcrc, (tmp16 >> 0) & 0xFF);
	...
	case SER_RDRAW:
		if (UART_TMT_NOEMPTY() > 10) {
			if (uart_tmt_pos < uart_tmt_len) {
				uart_tmt_datadle(&uart_tcrc,
						((uint16_t*) (pRaw))[uart_tmt_pos] >> 8);
				uart_tmt_datadle(&uart_tcrc,
						((uint16_t*) (pRaw))[uart_tmt_pos] & 0xFF);
				uart_tmt_pos++;
			} else {
				tmp16 = uart_tcrc;
				uart_tmt_datadle(&uart_tcrc, (tmp16 >> 0) & 0xFF);
	...
	case SER_RD_BBX:
		if (UART_TMT_NOEMPTY() > 10) {
			if (uart_tmt_pos < uart_tmt_len) {
				uart_tmt_datadle(&uart_tcrc,
						((uint16_t*) (bbox))[(uart_bbx_1st_data + uart_tmt_pos)
								% (BBOX_LEN * NO_CH * 2)] >> 8);
				uart_tmt_datadle(&uart_tcrc,
						((uint16_t*) (bbox))[(uart_bbx_1st_data + uart_tmt_pos)
								% (BBOX_LEN * NO_CH * 2)] & 0xFF);
				uart_tmt_pos++;
			} else {
				tmp16 = uart_tcrc;
				uart_tmt_datadle(&uart_tcrc, (tmp16 >> 0) & 0xFF);
