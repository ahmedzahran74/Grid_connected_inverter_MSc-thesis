...
uint16_t f_samp = F_SAMP; /*!< sampling frequency - should be in a spearate file */
...
const uint16_t noofBBXch = NO_CH;
uint32_t serial_nr = 12345;

uint32_t BuildDate = CURRENTDATE;
uint32_t BuildTime = CURRENTTIME;

extern uint16_t AI_reg[];
...
const var_type vars_misc[] = { { "en_fl_bbx", UNIT_NONE, TYP_U16, CA_U32(
		en_fl_bbx), 0, 0 }, /*!< enable floating point monitoring in blackbox for all DSP */
...
{ "s_date", UNIT_NONE, TYP_ENWR | TYP_32BIT, CA_U32(s_date), CA_U32(max_s_date),
		CA_U32(min_s_date) }, /*!< for date setup for all DSP */
{ "s_time", UNIT_NONE, TYP_ENWR | TYP_32BIT, CA_U32(s_time), CA_U32(max_s_time),
		CA_U32(min_s_time) }, /*!< for time setup for all DSP */
{ "set_dt", UNIT_NONE, TYP_ENWR | TYP_16BIT, CA_U32(set_datetm), CA_U32(
		max_set_datetm), CA_U32(min_set_datetm) }, /*!< for date,time setup for all DSP */
{ "save_var", UNIT_NONE, TYP_ENWR | TYP_16BIT, CA_U32(save_var), CA_U32(
		max_save_var), CA_U32(min_save_var) }, /*!< save variables for all DSP */
{ "load_defv", UNIT_NONE, TYP_ENWR | TYP_16BIT, CA_U32(load_defv), CA_U32(
		max_load_defv), CA_U32(min_load_defv) }, /*!< load default variable values for all DSP */
...
{ "bbxmemlen", UNIT_NONE, TYP_U32, CA_U32(bbx_mem_length), 0, 0 }, /*'< used to get the maximum record length in bbx */
};

/*! Titles of the disserent tabs: the 0th one is the Tab name, since it is not a real tab, but only used for the HiTerm.
 */
const uint8_t tabtitle[MON_MAXTABS][MAXCH_TABNAME + 1] = { { "SW28069" } };
...
const uint8_t units[][MAXCH_UNIT + 1] = { { "" }, { "A" }, { "V" }, { "kVA" }, {
		"kW" }, { "s" }, { "hour" }, { "Nm" }, { "kWh" }, { "Hz" }, { "Vs" }, {
		"NA" }, { "W" }, { "VAr" }, { "VA" }, { "rad/s" }, { "DI" }, { "rad" },
		{ "Ver" }, { "°C" }, { "%" }, { "BitF" }, { "100ms" }, { "ms" },
		{ "mA" }, };
const desc_type error_descriptor[LEN_ERR_DESC] = { { 0, "No error" }, { 1,
		"Overcurrent R" }, { 2, "Overcurrent S" }, { 3, "Overcurrent T" }, { 4,
		"Overv DC-link" }, { 5, "Overvoltage R" }, { 6, "Overvoltage S" }, { 7,
		"Overvoltage T" }, { 8, "IGBT sat prot R" }, { 9, "IGBT sat prot S" }, {
		10, "IGBT sat prot T" }, { 11, "Overload R" }, { 12, "Overload S" }, {
		13, "Overload T" }, { 44, "Unknown error" }, };

const desc_type mainsm_descriptor[LEN_MAINSM_DESC] = { { 0, "Stopped" }, { 1,
		"Starting" }, { 2, "Operational" }, { 3, "Waiting" }, { 99,
		"Crazy state" }, };
