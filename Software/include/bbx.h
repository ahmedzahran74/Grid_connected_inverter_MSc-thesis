	...
	#define BBOX_LEN  0x0200     /*!< length of the black box in x (time) direction) */
	#define NO_CH       8        /*!< number of channels used for data collecting (blackbox function) only implemented for 4*/
	...
	#if BBOX_USE_32BIT == 1
	#if USE_SPI_RAM == 0
	extern int32_t bbox[BBOX_LEN][NO_CH]; /*!< 32 bit buffer for storing the data */
	#else
	extern int32_t bbox[1][1]; /*!< 32 bit buffer for storing the data */
	#endif
	#else
	extern int16_t bbox[BBOX_LEN][NO_CH]; /*!< 16 bit buffer for storing the data */
	#endif
